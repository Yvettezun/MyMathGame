...
class Professional2 : AppCompatActivity(), View.OnClickListener {

    private var mCurrentPosition:Int=36
        ...
        when(v?.id){ //when an option is clicked,
            ...
            R.id.submit_next->{ //if submit is clicked
                if(mSelectedOptionPosition==0){//no option clicked?
                    ...
                    when{ mCurrentPosition<=mQuestionsList!!.size-5 ->{//theres still questions left
                    ...
                    if(mCurrentPosition!=40){ //checks if questions left, then changes button
