...
class Professional : AppCompatActivity(), View.OnClickListener {

    private var mCurrentPosition:Int=31
        ...
        when(v?.id){ //when an option is clicked,
            ...
            R.id.submit_next->{ //if submit is clicked
                if(mSelectedOptionPosition==0){//no option clicked?
                    ...
                    when{ mCurrentPosition<=mQuestionsList!!.size-10->{//theres still questions left
                    ...
                    if(mCurrentPosition!=35){ //checks if questions left, then changes button
